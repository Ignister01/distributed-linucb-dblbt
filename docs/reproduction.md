...
## Focused Restricted-Profile Confirmation

The final manuscript uses the protocol-gated experiment in
[experiments/comments0720/README.md](../experiments/comments0720/README.md).
It screens all 24 legal profiles on discovery seeds, warm-starts two online
profiles from 73,541 local samples, and evaluates eight alternating 4,096-round
phases over 32 untouched seeds. The primary phase-conditional result is
+0.007338 versus fixed TMC (95% paired-bootstrap CI
[+0.007093,+0.007584]; 32/32 positive). The same README gives the exact
commands for fitting the restricted model, running both policies, reducing
phase effects, and regenerating Figure 2.

The separate shadowing-enabled ns-3 diagnostic is retained at
[experiments/comments0720/results/ns3-shadowing-benchmark/](../experiments/comments0720/results/ns3-shadowing-benchmark/).
It is a five-second TMC measurement-path check (4+4, seed 410), not an
adaptive-policy comparison; its metrics.csv and audit.json record the
throughput, delay, packet-loss, access-overlap, schema, and shadowing settings.
